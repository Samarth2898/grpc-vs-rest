...
def dotproduct():
    ...
    try:
        ...
        if not (len(a) == len(b)):
            errorResponse = {
                "error": "Ensure the matrices are of same length"
            }
            return Response(response= jsonpickle.encode(errorResponse), status=400, mimetype="application/json")
        ...
        return Response(response="Bad Request, input not in the required format", status=400)
